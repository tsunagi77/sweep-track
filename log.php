...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt = $conn->prepare("SELECT email, password FROM supervisors WHERE email = ?");
    ...
    if ($result->num_rows > 0) {
        ...
        $password = $_GET['password'];
            ...
            header("Location: supervisorannounce.php");
            exit();
        if (!$password) {
...
<body>
    <div class="wrapper">
        ...
        <form id="login-form" method="POST" action=""><br>
            ...
            <div class="forget">
                <a href="supervisorforgot.php" id="forgot-password-link">Forgot password?</a>
            </div>
            <button type="submit">Log In</button>
            <div class="register">
                <p>Don't have an account? <a href="supervisorregister.php" id="show-register">Register</a></p>
            </div>
        </form>
        <form class="hidden" id="register-form" method="POST" action="supervisorregister.php"><br>
            <?php if (!empty($error_message)): ?>
                <div style="color: red;"><?= $error_message ?></div>
            <?php endif; ?>
            <div class="input-field">
                <span class="input-icon"><i class="fa-solid fa-id-card"></i></span>
                <input type="supervisorID" name="supervisorID" placeholder="Enter your Supervisor ID" required>
            </div>
            <div class="input-field">
    ...
    <div id="forgot-password-modal" class="modal">
        <div class="modal-content">
            ...
            <form method="POST" action="supervisorforgot.php">
